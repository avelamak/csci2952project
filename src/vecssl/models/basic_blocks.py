...
class FCN(nn.Module):
    ...
    def forward(self, out):
        ...
        args_logits = args_logits.reshape(
            S, N, self.n_args, self.args_dim
        )  # Shape [S, N, n_args, args_dim]
...
class ResNet(nn.Module):
    def __init__(self, d_model):
        ...
        self.linear1 = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU())
        self.linear2 = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU())
        self.linear3 = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU())
        self.linear4 = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU())
