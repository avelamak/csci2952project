...
def _get_key_padding_mask(commands, seq_dim=0):
    ...
    """
    with torch.no_grad():
        key_padding_mask = (commands == SVGTensor.COMMANDS_SIMPLIFIED.index("EOS")).cumsum(
            dim=seq_dim
        ) > 0
...
def _get_padding_mask(commands, seq_dim=0, extended=False):
    with torch.no_grad():
        padding_mask = (commands == SVGTensor.COMMANDS_SIMPLIFIED.index("EOS")).cumsum(
            dim=seq_dim
        ) == 0
...
def _get_visibility_mask(commands, seq_dim=0):
    ...
    S = commands.size(seq_dim)
    with torch.no_grad():
        visibility_mask = (commands == SVGTensor.COMMANDS_SIMPLIFIED.index("EOS")).sum(
            dim=seq_dim
        ) < S - 1
...
def _get_key_visibility_mask(commands, seq_dim=0):
    S = commands.size(seq_dim)
    with torch.no_grad():
        key_visibility_mask = (commands == SVGTensor.COMMANDS_SIMPLIFIED.index("EOS")).sum(
            dim=seq_dim
        ) >= S - 1
