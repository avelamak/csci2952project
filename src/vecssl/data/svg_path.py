...
from .geom import *
import vecssl.data.geom as geom
import re
import torch
...
from vecssl.data.geom import union_bbox
from vecssl.data.svg_command import (
    SVGCommand,
    SVGCommandMove,
    SVGCommandClose,
    SVGCommandBezier,
...
from vecssl.data.svg_primitive import SVGPathGroup
...
class SVGPath:
    def __init__(
        self,
        path_commands: Optional[List[SVGCommand]] = None,
        origin: Point = None,
        closed=False,
    ...
    def from_xml(x: minidom.Element):
        ...
        filling = (
            Filling.OUTLINE if not x.hasAttribute("filling") else int(x.getAttribute("filling"))
        )
    ...
    def from_str(s: str, fill=False, filling=Filling.OUTLINE, add_closing=False):
        ...
        for cmd, args in SVGPath._tokenize_path(s):
            cmd_parsed, pos, initial_pos = SVGCommand.from_str(
                cmd, args, pos, initial_pos, prev_command
            )
            prev_command = cmd_parsed[-1]
            path_commands.extend(cmd_parsed)

        return SVGPath.from_commands(
            path_commands, fill=fill, filling=filling, add_closing=add_closing
        )

    @staticmethod
    def from_tensor(tensor: torch.Tensor, allow_empty=False):
        return SVGPath.from_commands(
            [SVGCommand.from_tensor(row) for row in tensor], allow_empty=allow_empty
        )
        ...
        for command in path_commands:
            if isinstance(command, SVGCommandMove):
                if svg_path is not None and (
                    allow_empty or svg_path.path_commands
                ):  # SVGPath contains at least one command
                    if add_closing:
    ...
    def _get_viz_elements(
        ...
        # Note: with_bboxes is ignored for individual paths (only used at group level)
        return [*points, *handles]

    def draw(self, viewbox=None, *args, **kwargs):
    ...
    def translate(self, vec):
        for geo in self._get_unique_geoms():
            geo.translate(vec)
        return self
    ...
    def duplicate_extremities(self):
        self.path_commands = [
            SVGCommandLine(self.start_pos, self.start_pos),
            *self.path_commands,
    ...
    def reorder(self):
        if self.closed:
            _, topleftmost_idx = self._get_topleftmost_command()
    ...
    def to_video(self, wrapper, clips=None, svg_commands=None, color="grey"):
        ...
        for command in self.all_commands():
            ...
            im = SVG([*svg_paths, *svg_moves, *svg_dots]).draw(
                do_display=False, return_png=True, with_points=False
            )
    ...
    def simplify(self, tolerance=0.1, epsilon=0.1, angle_threshold=179.0, force_smooth=False):
        ...
        def generateBezier(first, last, uPrime, tan1, tan2):
            ...
            if abs(detC0C1) > epsilon:
                ...
                c1 = C[1, 0] + C[1, 1]
                alpha1 = alpha2 = (
                    X[0] / c0 if abs(c0) > epsilon else (X[1] / c1 if abs(c1) > epsilon else 0)
                )
        ...
        if force_smooth:
            ...
            if segments_list:
                ...
                for seg, seg_next in zip(segments_list[:-1], segments_list[1:], strict=False):  # noqa: RUF007
        ...
        for command in self.path_commands:
            if isinstance(command, SVGCommandLine) and not include_lines:
                ...
                if max_dist is not None:
                    n = max(math.ceil(cmd_len / max_dist), 1)
