...
from vecssl.data.svg_command import SVGCommandBezier
from vecssl.data.svg_path import SVGPath, Filling, Orientation
from vecssl.data.svg_primitive import (
    SVGPathGroup,
    SVGRectangle,
    SVGCircle,
    SVGEllipse,
...
class SVG:
    ...
    def copy(self):
        return SVG(
            [svg_path_group.copy() for svg_path_group in self.svg_path_groups], self.viewbox.copy()
        )

    @staticmethod
    def load_svg(svg_code):
        return SVG.from_str(svg_code)
    ...
    def from_tensors(tensors: List[torch.Tensor], viewbox: Bbox = None, allow_empty=False):
        ...
        svg = SVG(
            [SVGPath.from_tensor(t, allow_empty=allow_empty) for t in tensors], viewbox=viewbox
        )
        return svg
    ...
    def to_str(
        ...
        newline = "\n"
        return (
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{self.viewbox.to_str()}" \
            height="200px" width="200px">'
            f"{self._markers() if with_markers else ''}"
            f"{
                newline.join(
                    svg_path_group.to_str(fill=fill, with_markers=with_markers)
    ...
    def animate(self, file_path=None, frame_duration=0.1, do_display=True):
        ...
        if do_display:
            ...
            ipd.display(
                ipython_display(src, fps=24, rd_kwargs=dict(logger=None), autoplay=1, loop=1)  # noqa: C408
            )
    ...
    def simplify_heuristic(self, tolerance=0.1, force_smooth=False):
        return (
            ...
            .simplify(
                tolerance=tolerance, epsilon=0.2, angle_threshold=150, force_smooth=force_smooth
            )
            .split(max_dist=7.5)
        )
    ...
    def unit_circle():
        ...
        circle = SVGPath(
            [
                SVGCommandBezier(
                    Point(0.5, 0.0), Point(0.5 + d, 0.0), Point(1.0, 0.5 - d), Point(1.0, 0.5)
                ),
                SVGCommandBezier(
                    Point(1.0, 0.5), Point(1.0, 0.5 + d), Point(0.5 + d, 1.0), Point(0.5, 1.0)
                ),
                SVGCommandBezier(
                    Point(0.5, 1.0), Point(0.5 - d, 1.0), Point(0.0, 0.5 + d), Point(0.0, 0.5)
                ),
                SVGCommandBezier(
                    Point(0.0, 0.5), Point(0.0, 0.5 - d), Point(0.5 - d, 0.0), Point(0.5, 0.0)
                ),
            ]
        ).to_group()
                ...
                while current:
                    ...
                    if erase_neighbors:
                        for n in erase_neighbors:
                            neighbor = (
                                self[n].path.copy().set_orientation(Orientation.COUNTER_CLOCKWISE)
                            )
