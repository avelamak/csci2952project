...
class SVGEllipse(SVGPrimitive):
    ...
    def to_str(self):
        ...
        return f'<ellipse {fill_attr} cx="{self.center.x}" cy="{self.center.y}" \
        rx="{self.radius.x}" ry="{self.radius.y}"/>'
    ...
    def to_path(self):
        ...
        from vecssl.data.svg_command import SVGCommandArc
...
class SVGCircle(SVGEllipse):
    ...
    def to_str(self):
        fill_attr = self._get_fill_attr()
        return (
            f'<circle {fill_attr} cx="{self.center.x}" cy="{self.center.y}" r="{self.radius.x}"/>'
        )
...
class SVGRectangle(SVGPrimitive):
    ...
    def to_str(self):
        fill_attr = self._get_fill_attr()
        return f'<rect {fill_attr} x="{self.xy.x}" y="{self.xy.y}" \
            width="{self.wh.x}" height="{self.wh.y}"/>'
    ...
    def to_path(self):
        from vecssl.data.svg_path import SVGPath
        from vecssl.data.svg_command import SVGCommandLine

        p0, p1, p2, p3 = (
            self.xy,
            self.xy + self.wh.xproj(),
            self.xy + self.wh,
            self.xy + self.wh.yproj(),
        )
        commands = [
...
class SVGLine(SVGPrimitive):
    ...
    def to_str(self):
        fill_attr = self._get_fill_attr()
        return f'<line {fill_attr} x1="{self.start_pos.x}" y1="{self.start_pos.y}" \
...
class SVGPolyline(SVGPrimitive):
    ...
    def to_str(self):
        fill_attr = self._get_fill_attr()
        return '<polyline {} points="{}"/>'.format(
            fill_attr, " ".join([p.to_str() for p in self.points])
        )
    ...
    def from_xml(cls, x: minidom.Element):
        ...
        assert len(args) % 2 == 0, (
            f"Expected even number of arguments for SVGPolyline: {len(args)} given"
        )
    ...
    def to_path(self):
        ...
        commands = [
            SVGCommandLine(p1, p2)
            for p1, p2 in zip(self.points[:-1], self.points[1:], strict=False)
        ]
...
class SVGPathGroup(SVGPrimitive):
    def __init__(self, svg_paths: Optional[List[SVGPath]] = None, origin=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def __repr__(self):
        return "SVGPathGroup({})".format(
            ", ".join(svg_path.__repr__() for svg_path in self.svg_paths)
        )
    ...
    def to_str(self, fill=None, with_markers=False):
        # Note: fill parameter is ignored - uses self.fill instead
        fill_attr = self._get_fill_attr()
                        ...
                        for n2 in G.neighbors(n):
                            if n2 not in visited:
                                d2 = (
                                    d
                                    + (
                                        self.svg_paths[n2].is_clockwise()
                                        == self.svg_paths[n].is_clockwise()
                                    )
                                    * 2
                                    - 1
                                )
