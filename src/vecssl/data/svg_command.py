...
from vecssl.data.geom import *
from vecssl.data.svg_tensor import SVGTensor
from vecssl.data.util_fns import get_roots
...
class SVGCommand:
    ...
    def from_str(
        ...
        assert nb_args % expected_nb_args == 0, (
        ...
        for _ in range(nb_args // expected_nb_args):
            ...
            if (
                cmd is SVGCmdEnum.LINE_TO
                or cmd is SVGCmdEnum.LINE_TO_VERTICAL
                or cmd is SVGCmdEnum.LINE_TO_HORIZONTAL
            ):
            ...
            cmds_parsed.append(cmd_parsed)

        return cmds_parsed, pos, initial_pos
    ...
    def from_args(
        ...
        if command is SVGCmdEnum.MOVE_TO:
            ...
            return SVGCommandArc(
                start_pos, radius, x_axis_rotation, large_arc_flag, sweep_flag, end_pos
            )
        return None
    ...
    def get_points_viz(self, first=False, last=False):
        from vecssl.data.svg_primitive import SVGCircle
...
class SVGCommandMove(SVGCommandLinear):
    ...
    def get_points_viz(self, first=False, last=False):
        from vecssl.data.svg_primitive import SVGLine
...
class SVGCommandLine(SVGCommandLinear):
    ...
    def sample_points(self, n=10, return_array=False):
        ...
        if return_array:
            points = (1 - z)[:, None] * self.start_pos.pos[None] + z[:, None] * self.end_pos.pos[
                None
            ]
            return points
    ...
    def split(self, n=2):
        ...
        return [SVGCommandLine(p1, p2) for p1, p2 in zip(points[:-1], points[1:], strict=False)]  # noqa: RUF007
...
class SVGCommandBezier(SVGCommand):
    ...
    def copy(self):
        return SVGCommandBezier(
            self.start_pos.copy(), self.control1.copy(), self.control2.copy(), self.end_pos.copy()
        )
    ...
    def from_vector(vector):
        return SVGCommandBezier(
            Point(vector[0]), Point(vector[1]), Point(vector[2]), Point(vector[3])
        )
    ...
    def get_handles_viz(self):
        from vecssl.data.svg_primitive import SVGLine, SVGCircle

        anchor_1 = SVGCircle(
            self.control1, radius=Radius(0.4), color="lime", fill=True, stroke_width=".1"
        )
        anchor_2 = SVGCircle(
            self.control2, radius=Radius(0.4), color="lime", fill=True, stroke_width=".1"
        )

        handle_1 = SVGLine(
            self.start_pos, self.control1, color="grey", dasharray=0.5, stroke_width=".1"
        )
        handle_2 = SVGLine(
            self.end_pos, self.control2, color="grey", dasharray=0.5, stroke_width=".1"
        )
    ...
    def sample_points(self, n=10, return_array=False):
        ...
        Q = np.array(
            [[1.0, 0.0, 0.0, 0.0], [-3, 3.0, 0.0, 0.0], [3.0, -6, 3.0, 0.0], [-1, 3.0, -3, 1]]
        )
...
class SVGCommandArc(SVGCommand):
    def __init__(
        ...
        super().__init__(
            SVGCmdEnum.ELLIPTIC_ARC,
            [radius, x_axis_rotation, large_arc_flag, sweep_flag, end_pos],
            start_pos,
            end_pos,
        )
    ...
    def copy(self):
        return SVGCommandArc(
            self.start_pos.copy(),
            self.radius.copy(),
            self.x_axis_rotation.copy(),
            self.large_arc_flag.copy(),
            self.sweep_flag.copy(),
            self.end_pos.copy(),
        )
    ...
    def _get_center_parametrization(self):
        ...
        sqrt = math.sqrt(max((rx2 * ry2 - rx2 * y2 - ry2 * x2) / (rx2 * y2 + ry2 * x2), 0.0))
    ...
    def to_beziers(self):
        ...
        for eta_1, eta_2 in zip(etas[:-1], etas[1:], strict=False):  # noqa: RUF007
