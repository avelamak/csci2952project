"""
Test SVG dataloading performance with already_tensor=False
Tests loading .svg files (baseline for comparison with tensor loading)
"""
...
def test_single_sample_loading(svg_dir, img_dir, meta_file, num_samples=50):
...
def test_dataloader_throughput(svg_dir, img_dir, meta_file, num_iterations=25):
...
def test_memory_usage(svg_dir, img_dir, meta_file, num_samples=50):
...
def main():
    # Set paths - .svg files are in svgx_svgs directory
    svg_dir = "/Users/jz/work/csci2952project/svgx_svgs_svg"
    img_dir = "/Users/jz/work/csci2952project/svgx_imgs_svg"
    meta_file = "/Users/jz/work/csci2952project/svgx_meta_svg.csv"
    output_file = "/Users/jz/work/csci2952project/jz_test/20251119_test_tensor_dataset/svg_benchmark_results.csv"

    print("="*80)
    print("SVG DATALOADING PERFORMANCE TEST (Baseline)")
    print("="*80)
    print(f"\nConfiguration:")
    print(f"  SVG Directory (.svg files): {svg_dir}")
    print(f"  Image Directory:            {img_dir}")
    print(f"  Metadata File:              {meta_file}")

    # Check if directories exist
    if not os.path.exists(svg_dir):
        print(f"\nERROR: SVG directory not found: {svg_dir}")
        return
    ...
    if not svg_files:
        ...
        print("\nTo create SVG files, run:")
        print("  python scripts/preprocess.py \\")
        print("    --output_meta_file svgx_meta.csv \\")
        print(f"    --output_svg_folder {svg_dir} \\")
        print(f"    --output_img_folder {img_dir} \\")
        print("    --max_samples 500")
        return

    print(f"\nFound {len(svg_files)} .svg files in directory")

    # Run tests
    try:
        single_results = test_single_sample_loading(svg_dir, img_dir, meta_file, num_samples=50)
        throughput_results = test_dataloader_throughput(svg_dir, img_dir, meta_file, num_iterations=25)
        memory_results = test_memory_usage(svg_dir, img_dir, meta_file, num_samples=50)
